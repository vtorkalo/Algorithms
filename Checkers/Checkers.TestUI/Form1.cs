...
namespace Checkers
{
    public partial class Form1 : Form
    {
        ...
        {
            _field = TestFieldData.Standart_Moves_Case5();
        ...
        private void pnlField_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < 32; i++)
            {
                Field.GetRowCol(i, out int y, out int x);
                int color = (x + (y + 1 % 2)) % 2;
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 32; i++)
            {
                Field.GetRowCol(i, out int row, out int col);
                var value = Field.GetValue(_field, row, col);
                if (value != CellState.Empty)
                {
                    builder.AppendLine(string.Format("Field.SetValue(result, {0}, {1}, CellState.{2});", row, col, value.ToString()));
                }
            }
            if (_selectedCell != null)
            {
